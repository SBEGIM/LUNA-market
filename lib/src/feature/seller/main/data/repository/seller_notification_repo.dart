...
class SellerrNotificationRepository {
  ...
  Future<List<NotificationSellerModel>> notifications() =>
...
class NotifactionToApi {
  ...
  Future<List<NotificationSellerModel>> notifications() async {
    ...
    final response = await http.get(
      Uri.parse(
          '$baseUrl/list/notifications?notificationable=App/Models/Seller'),
      headers: {"Authorization": "Bearer $token"},
    );
    final data = jsonDecode(response.body);

    return (data as List)
        .map((e) => NotificationSellerModel.fromJson(e))
        .toList();
  }
  ...
  Future<int> read(int id) async {
    ...
    final response = await http.get(
      Uri.parse(
          '$baseUrl/list/notifications/read?notificationable=App/Models/Seller&id=$id'),
      headers: {"Authorization": "Bearer $token"},
    );
  ...
  Future<int> count() async {
    ...
    final response = await http.get(
      Uri.parse(
          '$baseUrl/list/notifications/count?notificationable=App/Models/Seller'),
      headers: {"Authorization": "Bearer $token"},
    );
