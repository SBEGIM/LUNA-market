...
import 'package:haji_market/src/feature/seller/product/data/DTO/color_count_seller_dto.dart';
...
class _EditProductSellerPageState extends State<EditProductSellerPage> {
  ...
  void initState() {
    ...
    if (BlocProvider.of<BrandCubit>(context).state is! LoadedState) {
  ...
  void _charactisticsArray() async {
    ...
    widget.product.characteristics?.forEach((element) {
      characteristicsValue?.add(CharacteristicsModel(id: element.id, key: element.name));
  ...
  void dispose() {
    ...
    massaController.dispose();
    deepController.dispose();
    descriptionController.dispose();
    sizeCountController.dispose();
    sizePriceController.dispose();
    optomPriceController.dispose();
    optomCountController.dispose();
    optomController.dispose();
    pointsController.dispose();
    pointsBloggerController.dispose();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        centerTitle: true,
        title: const Text('Редактировать товар', style: AppTextStyles.appBarTextStyle),
        leading: Padding(
      ...
      body: Padding(
        ...
        child: ListView(
          ...
          children: [
                          ...
                          PopupMenuButton(
                            ...
                            itemBuilder: (BuildContext bc) {
                              return const [
                                PopupMenuItem(
                                  value: '₽',
                                  child: Text('Рубль, ₽', style: TextStyle(color: Colors.black)),
                                ),
                                PopupMenuItem(
                                  value: ' % ',
                                  child: Text('Процент, % ', style: TextStyle(color: Colors.black)),
                                ),
                              ];
                            },
                          ),
            ...
            if (filledCount == 1) const SizedBox(height: 80),
            SizedBox(height: 20),
            if (filledCount == 2)
              SizedBox(
                child: Column(
                ...
                onPressed: () {
                  ...
                  showSellerCharacteristicsOptions(context, 'Укажите цвета', dataWidget, (
                    ...
                    for (var e in values) {
                      ...
                      final index = colorCount.indexWhere((element) => element.color_id == colorId);
                      if (index != -1) {
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (filledCount == 4) const SizedBox(height: 10),
                    if (filledCount == 4)
                      Row(
                        children: [
                          GestureDetector(
                          ...
                          _image.isNotEmpty
                              ? SizedBox(
                                  ...
                                  child: ListView.builder(
                                    ...
                                    itemExtent: 100, // Fixed width for each item
                                    physics:
                                        const ClampingScrollPhysics(), // Better control than Bouncing
                                    padding: EdgeInsets.zero,
                                    shrinkWrap: false, // Important for scrolling
                                    primary: false, // Приятная физика прокрутки
                                    itemBuilder: (context, index) {
                                      return Padding(
                                        ...
                                        child: Stack(
                                          clipBehavior:
                                              Clip.none, // Позволяет иконке выходить за границы
                                          children: [
                    ...
                    Row(
                      children: [
                        ...
                        if (_video != null &&
                          ...
                          Padding(
                            padding: const EdgeInsets.only(right: 8), // Отступ между изображениями
                            child: Container(
                              height: 92,
                              width: 92,
                              decoration: BoxDecoration(
            ...
            if (filledCount == 4) const SizedBox(height: 20),

            if (filledCount == 4) const SizedBox(height: 80),
                              ...
                              child: AnimatedContainer(
                                ...
                                decoration: BoxDecoration(
                                  ...
                                  color: !isCheckedFBS
                                      ...
                                      : Colors.transparent,
                                  border: Border.all(
                                    color: !isCheckedFBS ? AppColors.mainPurpleColor : Colors.grey,
                                    width: 2,
                                  ),
                                ),
                                child: !isCheckedFBS
            ...
            if (filledCount == 6 &&
              ...
              FieldsProductRequest(
                ...
                hintColor: false,
                onPressed: () {
                  if (_locationSelect == 'Регион' || _locationSelect == 'Город/населенный пункт') {
            ...
            if (filledCount == -1) const SizedBox(height: 20),
            if (filledCount == -1)
              SizedBox(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                                        ...
                                        children: [
                                          ...
                                          PopupMenuButton(
                                            onSelected: (value) async {
                                              characteristicsValuelast = CharacteristicsModel(
                                                id: value.id,
                                                key: value.key,
                                              );
                                              //sizeId = value.id.toString();
                                    ...
                                    child: Container(
                                      ...
                                      child: Row(
                                        ...
                                        children: [
                                          ...
                                          PopupMenuButton(
                                            onSelected: (value) {
                                              subCharacteristicsValueLast = CharacteristicsModel(
                                                id: value.id,
                                                value: value.value,
                                              );
            ...
            if (filledCount == -1) const SizedBox(height: 15),
            if (filledCount == -1)
              SizedBox(
            ...
            if (filledCount == -1) const SizedBox(height: 10),
            if (filledCount == -1)
              const Text(
            ...
            if (filledCount == -1) const SizedBox(height: 8),
            if (filledCount == -1)
              Container(
                width: 166,
                padding: const EdgeInsets.symmetric(horizontal: 10),
            ...
            if (filledCount == -1) const SizedBox(height: 28),
            if (filledCount == -1)
              SizedBox(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
      ...
      bottomNavigationBar: BlocConsumer<ProductSellerCubit, ProductAdminState>(
        listener: (context, state) {
