...
class ChangePasswordSellerPage extends StatefulWidget {
  ...
  const ChangePasswordSellerPage({super.key, required this.textEditingController});

  @override
  State<ChangePasswordSellerPage> createState() => _ChangePasswordSellerPageState();
}
      ...
      body: BlocConsumer<SmsSellerCubit, SmsSellerState>(
        listener: (context, state) {
          if (state is ResetSuccessState) {
            ...
            // );
            context.router.popUntil((route) => route.settings.name == AuthSellerRoute.name);
          }
        },
