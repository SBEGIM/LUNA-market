...
abstract interface class IAuthRemoteDS {
  ...
  //
  Future<List<String>?> sendDeviceToken({required String deviceToken});
...
class AuthRemoteDSImpl implements IAuthRemoteDS {
  ...
  Future<UserDTO> login({required String phone, required String password}) async {
    try {
      ...
      if (response case {
        ...
        final user = UserDTO.fromJson(userObject).copyWith(accessToken: accessToken);
        return user;
      } else {
  ...
  @override
  Future<List<String>?> sendDeviceToken({required String deviceToken}) async {
    try {
      final response = await restClient.put('/v1/user/extra', body: {'device_token': deviceToken});

      if (response['push_notification_topics'] != null &&
          response['push_notification_topics'] is List) {
        return (response['push_notification_topics']! as List).mapToList((e) => e as String);
      } else {
        return throw WrongResponseTypeException(
          message: '''Unexpected response body type: ${response.runtimeType}\n$response''',
        );
  ...
  Future<String> generateCentrifugoToken() async {
    try {
      final Map<String, dynamic> response = await restClient.post('/v1/centrifugo/auth', body: {});
