...
class BloggerProductStatisticsCubit extends Cubit<BloggerProductStatisticsState> {
  final BloggerProductsStatisticsRepository bloggerProductStatisticsRepository;

  BloggerProductStatisticsCubit({required this.bloggerProductStatisticsRepository})
    : super(InitState());

  Future<void> statistics() async {
    try {
      emit(LoadingState());
      final BloggerProductStatisticsModel data = await bloggerProductStatisticsRepository
          .statistics();
