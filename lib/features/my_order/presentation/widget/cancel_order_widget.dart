...
class _CancelOrderWidgetState extends State<CancelOrderWidget> {
  ...
  List<String> cancel = ['Не устрайвает сроки', 'Товара нет в наличи', 'Продовец попросил отменить', 'Другое'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      bottomSheet: BlocConsumer<OrderStatusAdminCubit, OrderStatusAdminState>(listener: (context, state) {
        if (state is CancelState) {
          int count = 0;
          Navigator.of(context).popUntil((_) => count++ >= 2);
          BlocProvider.of<BasketCubit>(context).basketOrderShow();
          Get.snackbar('Заказ', 'Заказ успешно отменен', backgroundColor: Colors.blueAccent);
        ...
        return Container(
          ...
          child: InkWell(
            onTap: () async {
              if (selectIndex == -1) {
                Get.snackbar('Ошибка', 'Выберите причину отказа!', backgroundColor: Colors.red);
              } else {
                BlocProvider.of<OrderStatusAdminCubit>(context).cancelOrder(widget.id, 'cancel', cancel[selectIndex]);
              }
            },
            child: Container(
