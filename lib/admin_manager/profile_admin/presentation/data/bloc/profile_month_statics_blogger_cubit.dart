...
import '../../../../../bloger/profile_admin/presentation/data/model/profile_month_blogger_statics.dart';
...
class ProfileMonthStaticsBloggerCubit extends Cubit<ProfileMonthStaticsBloggerState> {
  ...
  ProfileMonthStaticsBloggerCubit({required this.profileMonthStaticsBloggerRepository}) : super(InitState());

  Future<void> statics(int month, int year) async {
    try {
      emit(LoadingState());
      final List<ProfileMonthStatics> data = await profileMonthStaticsBloggerRepository.statics(month, year);
